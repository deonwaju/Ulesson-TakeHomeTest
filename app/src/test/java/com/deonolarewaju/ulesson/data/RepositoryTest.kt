...
class RepositoryTest {
    ...
    fun `assert that call to network returns success`() = mainCoroutineRule.runBlockingTest {
        ...
        `when`(remoteDataSource.getSubjectData()).thenReturn(Resources.success(TestObjectUtil.subjectDataResponse))
    ...
    fun `assert that when call to network fails it returns the appropriate error message`() =
        mainCoroutineRule.runBlockingTest {
            `when`(remoteDataSource.getSubjectData()).thenReturn(Resources.error("error occurred"))
